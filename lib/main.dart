...
void main() async {
  ...
  // Draw behind the status & navigation bars so they take on the app's own
  // background per theme instead of a system grey bar. Each screen refines the
  // icon brightness via AnnotatedRegion.
...
class _AIBuddyAppState extends State<AIBuddyApp> {
  ...
  void _applyAccentColor() {
    AppColors.primary = _settings.accentColor;
    AppColors.primaryDark = _settings.accentColor;
    setState(() {});
  }
  ...
  Future<void> _initServices() async {
    try {
      ...
      _toolRegistry = ToolRegistry.createDefault(
        ...
        imapServer: _secureConfig.imapServer,
        imapPort: _secureConfig.imapPort,
        emailAddress: _secureConfig.emailAddress,
        emailPassword: _secureConfig.emailPassword,
        imapUseSsl: _secureConfig.imapUseSsl,
      );
  ...
  Widget build(BuildContext context) {
    if (_error != null) {
      return MaterialApp(title: 'AI-Buddy', theme: AppTheme.light(AppColors.primary), darkTheme: AppTheme.dark(AppColors.primary), themeMode: ThemeMode.system, home: _ErrorScreen(error: _error!, onRetry: () { setState(() => _error = null); _initServices(); }));
    }
    if (!_initialized) {
      return MaterialApp(title: 'AI-Buddy', theme: AppTheme.light(AppColors.primary), darkTheme: AppTheme.dark(AppColors.primary), themeMode: ThemeMode.system, home: const _StartupScreen());
    }
    return MultiProvider(
      ...
      child: Consumer<SettingsService>(
        builder: (context, settings, _) {
          ...
          final langCode = settings.appLanguage;
  ...
  Widget build(BuildContext context) => Scaffold(
    body: Container(
      ...
      child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(width: 32, height: 32, child: CircularProgressIndicator(strokeWidth: 2.5, color: AppColors.primary)),
      ])),
    ),
  );
}
