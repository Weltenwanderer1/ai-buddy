...
class ManageShoppingListTool implements ToolInterface {
  ...
  static const _definition = ToolDefinition(
    ...
    parametersSchema: {
      ...
      'properties': {
        ...
        'launch_bring': {
          'type': 'boolean',
          'description': 'Falls wahr, wird der Artikel per Share-Intent an die Bring! App gesendet. Setze true, wenn der Nutzer Bring! verwendet.',
        }
      },
      'required': ['action'],
    },
  ...
  Future<ToolResult> execute(Map<String, dynamic> parameters) async {
    ...
    // Nur beim Hinzufügen teilen: bei remove würde der gerade entfernte
    // Artikel in Bring! neu angelegt, bei list/clear ein leerer Text geteilt.
    if (launchBring && action == 'add' && item.isNotEmpty) {
      try {
        // Try sharing the item text directly to Bring! via ACTION_SEND.
        // Bring! can receive shared text and add it as a list item.
        final shared = await AndroidAppLauncherService.shareToApp(_bringPackage, item);
        if (shared) {
          resultText += ' Bring! App wurde geöffnet und "$item" wurde geteilt — bitte in der App bestätigen.';
        } else {
          // Fallback: just open the app
          final launched = await AndroidAppLauncherService.launchApp(_bringPackage);
          if (launched) {
            resultText += ' Bring! App wurde gestartet (Share nicht verfügbar). Artikel manuell hinzufügen: $item';
          } else {
