  ...
  Future<void> _clear() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Löschen?', style: TextStyle(color: context.buddy.t1)),
        content: Text(
          '${_regionInfo?['tileCount'] ?? 'Alle'} Kacheln unwiderruflich löschen?',
          style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.7)),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: Text('Abbrechen', style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.5))),
          ),
          TextButton(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(children: [
              ...
              Text('Offline-Karten',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: context.buddy.t1)),
            ]),
            const SizedBox(height: 8),
            Text(
              'OpenStreetMap-Kacheln für Navigation ohne Internet.',
              style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.5), fontSize: 13),
            ),
                  ...
                  children: [
                    ...
                    if (_regionInfo?['lat'] != null && _regionInfo?['lon'] != null)
                      Text(
                        ...
                        style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.6), fontSize: 12),
                      ),
                    Text(
                    ...
                    if (updated.isNotEmpty)
                      Text(
                        ...
                        style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.4), fontSize: 11),
                      ),
                  ],
                ),
              ),
            ...
            if (_isDownloading || _isPaused) ...[
              ...
              Text('$_progressCurrent / $_progressTotal Kacheln',
                style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.6), fontSize: 13)),
              if (_isPaused)
            ...
            if (!_isDownloading) ...[
              ...
              Text('Bereich: ${_radiusLabel(_radiusDeg)}',
                style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.7), fontSize: 13)),
              Slider(
                value: _radiusDeg,
                min: 0.01,
                max: 0.1,
                ...
                inactiveColor: context.buddy.elev,
                ...
                children: [
                  Expanded(
                    child: Text('Zoom: $_zoomMin–$_zoomMax',
                      style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.5), fontSize: 12)),
                  ),
                ],
              ),
              const SizedBox(height: 4),
              Text(
                ...
                style: TextStyle(color: context.buddy.t1.withValues(alpha: 0.35), fontSize: 11),
              ),
            ],
            ...
            Wrap(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  style: ElevatedButton.styleFrom(
                    backgroundColor: context.buddy.elev,
                    foregroundColor: context.buddy.t1.withValues(alpha: 0.7),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                    padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 12),
                  ),
