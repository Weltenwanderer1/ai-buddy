...
class MemoryService extends ChangeNotifier {
  ...
  Future<void> addCore(String content, {String source = 'system', Map<String, dynamic>? metadata}) async {
    ...
    if (trimmed.length < 5) return; // Reject fragments
    // Check for near-duplicates
    if (_core.any((m) => _similarity(m.content, trimmed) > 0.85)) return;
  ...
  Future<void> addLongTerm(String content, {String source = 'system', Map<String, dynamic>? metadata}) async {
    ...
    if (trimmed.length < 5) return; // Reject fragments
    // Check for near-duplicates
    if (_longTerm.any((m) => _similarity(m.content, trimmed) > 0.85)) return;
    ...
    notifyListeners();
    await _saveLongTerm();
    _generateEmbeddingForLatest(trimmed);
  }
  ...
  List<MemoryItem> _searchWithEmbeddings(List<MemoryItem> tier, String query, List<double> queryEmbedding, {int limit = 5}) {
    final scored = <({MemoryItem item, double score})>[];
    for (final mem in tier) {
      ...
      if (mem.embedding != null && mem.embedding!.isNotEmpty) {
        ...
        // Memories ohne Embedding: Token-Overlap mit der QUERY als Fallback
        // (vorher wurde der Leerstring verglichen → sim war konstant 0 und
        // solche Memories konnten inhaltlich nie matchen).
        sim = _similarity(query, mem.content) * 0.5;
      }
