...
class PersonaEvolutionService extends ChangeNotifier {
  ...
  Future<void> importData(Map<String, dynamic> data) async {
    ...
    notifyListeners();
    // Ohne Persistierung wäre ein Backup-Restore nach dem nächsten
    // App-Neustart wieder weg.
    await _save();
  }

  void parseEvolutionResponse(String response) => _parseEvolutionResponse(response);

  /// Add a single trait (full sentence) — used by structured evolution.
  void addTrait(String trait) {
    final trimmed = trait.trim();
    if (trimmed.length < 10) return;
    if (_learnedTraits.contains(trimmed)) return;
    _learnedTraits.add(trimmed);
    notifyListeners();
    _save();
  }

  /// Add an avoid topic — used by structured evolution.
  void addAvoidTopic(String topic) {
    final trimmed = topic.trim();
    if (trimmed.isEmpty || _avoidTopics.contains(trimmed)) return;
    _avoidTopics.add(trimmed);
    notifyListeners();
    _save();
  }

  /// Add a preferred style — used by structured evolution.
  void addPreferredStyle(String style) {
    final trimmed = style.trim();
    if (trimmed.isEmpty || _preferredStyle.contains(trimmed)) return;
    _preferredStyle.add(trimmed);
    notifyListeners();
    _save();
  }
