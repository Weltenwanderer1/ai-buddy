...
/// Proactive check: Calendar heads-up, evening recap, POI geofencing, etc.
Future<bool> _runProactiveCheck(Map<String, dynamic>? input) async {
  try {
    ...
    if (hour >= 20 && hour < 23) {
      ...
      if (!alreadyFired) {
        ...
        await _showNotification(
          ...
          actions: [
            const AndroidNotificationAction('open_app', 'Öffnen'),
            const AndroidNotificationAction('dismiss', 'Später'),
          ],
        );
      }
    }
    ...
    if (hour >= 7 && hour < 9) {
      ...
      if (!alreadyFired) {
        ...
        await _showNotification(
          ...
          actions: [
            const AndroidNotificationAction('check_calendar', 'Termine checken'),
            const AndroidNotificationAction('dismiss', 'Später'),
          ],
        );
      }
    ...
    // Location-based POI check (uses cached location)
    final locJson = prefs.getString('last_location');
    if (locJson != null) {
      try {
        final loc = jsonDecode(locJson) as Map<String, dynamic>;
        final lat = (loc['lat'] as num).toDouble();
        final lon = (loc['lon'] as num).toDouble();
        final poisRaw = prefs.getString('proactive_pois');
        if (poisRaw != null) {
          final pois = jsonDecode(poisRaw) as List<dynamic>;
          for (final p in pois) {
            final map = p as Map<String, dynamic>;
            final plat = (map['lat'] as num).toDouble();
            final plng = (map['lng'] as num).toDouble();
            final radius = ((map['radiusMeters'] as num?)?.toDouble() ?? 200);
            final dist = _haversine(lat, lon, plat, plng);
            if (dist <= radius) {
              final name = map['name'] as String;
              final today = DateTime.now().toIso8601String().substring(0, 10);
              final key = 'proactive_poi_bg_${name}_$today';
              if (prefs.getString(key) != today) {
                await prefs.setString(key, today);
                final action = map['suggestedAction'] as String? ?? '📍 Du bist bei $name. Brauchst du was?';
                await _showNotification(
                  id: 9000 + name.hashCode % 999,
                  title: '📍 $name',
                  body: action,
                  actions: [
                    const AndroidNotificationAction('open_app', 'Öffnen'),
                    const AndroidNotificationAction('poi_done', 'Erledigt'),
                  ],
                );
              }
            }
        ...
        debugPrint('BuddyScheduler POI check error: $e');
      }
    }
...
Future<void> _showNotification({required int id, required String title, required String body, List<AndroidNotificationAction> actions = const []}) async {
...
double _haversine(double lat1, double lon1, double lat2, double lon2) {
  ...
  final dLat = (lat2 - lat1) * 0.01745329252;
  final dLon = (lon2 - lon1) * 0.01745329252;
  final a = 
      (0.5 - 0.5 * cos(dLat)) +
      cos(lat1 * 0.01745329252) * cos(lat2 * 0.01745329252) * (0.5 - 0.5 * cos(dLon));
...
class BuddyScheduler with ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    _tasks['proactive_check'] = BuddyTaskConfig(
      ...
      description: 'Morgengruß, Abendrückblick, Ort, Timer, Wetter',
      frequency: const Duration(minutes: 15), // alle 15 Min für echte Proximity
