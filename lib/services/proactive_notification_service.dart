...
class ProactiveNotificationService extends ChangeNotifier {
  ...
  void _onNotificationResponse(NotificationResponse response) {
    ...
    if (response.payload != null && response.payload!.isNotEmpty) {
      try {
        ...
        // Body-tap (no action button) → open chat with the notification text
        if (response.actionId == null || response.actionId!.isEmpty) {
          _onBodyTap(notification);
  ...
  Future<void> _onBodyTap(ProactiveNotification notification) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('proactive_pending_reply', notification.body);
    } catch (e) {
      debugPrint('_onBodyTap error: $e');
    }
  }
